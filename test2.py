...
# imagepath = r'C:\projects\MSIL_Maruti_Suzuki\msil_maruti_suzuki\images'
diff = {0: "Processing", 1: "Processing", 2: "Processing", 3: "Processing"}
diffpercent = {0: "Processing", 1: "Processing",
               2: "Processing", 3: "Processing"}
d = {0: [], 1: [], 2: []}

# UI FONT specifications - Font size decreased by 0.5 unit
font = cv2.FONT_HERSHEY_SIMPLEX
fontScale = 0.9  # Modified line: decreased from 1.1 to 0.9
...
def create_composite_frame(frame, box1, box2, box3, box4):
    # Resized dimensions adjusted for the new font size
    h, w = frame.shape[:2]
    aspect_ratio = w / h
    new_width = 960
    ...
    main_frame_padded[y_offset:y_offset + new_height, x_offset:x_offset + new_width] = main_frame_resized
...
def frameInf(frame):
    ...
    frame = claw_results[0].plot(labels=False)  
    ...
    if (click_position == 0 or click_position == 1) and 0 in all_cls:
        for i, val in enumerate(all_cls):
            if val == 0 and 350 <= all_xy[i][1] < 370:
                click_position += 1
                ...
                break
            elif val == 0 and click_position == 1 and 135 <= all_xy[i][1] < 155:
                click_position += 1
    ...
    if click_position != 0 and 0 in all_cls:
        for i, val in enumerate(all_cls):
            if val == 0 and 400 <= all_xy[i][1] < 540:
                click_position = 0
                diffpercent = {0: "Processing", 1: "Processing",
                               2: "Processing", 3: "Processing"}
    ...
    cv2.putText(img, "Video Feed", (285, 200), font, fontScale, color, thickness, cv2.LINE_AA)
...
def get_model_images(model_id):
    ...
    if model_id not in model_prefix_types:
